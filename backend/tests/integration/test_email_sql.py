...
def test_email_sql_validation_valid_cases():
    ...
    for email in cases:
        ...
        assert result[0]["IsInvalid"] == 0, (
            f"Expected {email} to be VALID, but SQL flagged it as INVALID."
        )
...
def test_email_sql_validation_invalid_cases():
    ...
    for email in INVALID_EMAILS:
        ...
        """
        result = execute_readonly_query(query)
        assert result[0]["IsInvalid"] == 1, (
            f"Expected {email} to be INVALID, but SQL flagged it as VALID."
        )
