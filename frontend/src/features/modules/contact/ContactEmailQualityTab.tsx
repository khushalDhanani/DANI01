...
export function ContactEmailQualityTab() {
  ...
  if (isLoading) {
    return (
      <View className="py-8 items-center justify-center">
        ...
        <Text className="text-sm text-slate-400 mt-3 font-medium">Auditing email quality & integrity rules...</Text>
      </View>
    );
  }
  ...
  if (isError || !quality) {
    return (
      <View className="py-8 items-center justify-center">
        ...
        <Text className="text-sm text-red-400 mt-3 font-medium">Failed to load email quality audit results.</Text>
      </View>
    );
  }

  // Filter email-related rules
  const emailRuleCodes = [
    "DUPLICATE_COMPANY_EMAIL",
    "DUPLICATE_PERSONAL_EMAIL",
    "INVALID_EMAIL_FORMAT",
    "PERSONAL_EMAIL_IN_COMPANY_FIELD",
    "SUSPICIOUS_PLACEHOLDER_EMAIL",
    "MISSING_ANY_EMAIL",
    "MISSING_COMPANY_EMAIL",
  ];

  const emailRules = quality.rules.filter((r) => emailRuleCodes.includes(r.rule_code));
  const filteredRules =
    selectedSeverity === "ALL"
      ? emailRules
      : emailRules.filter((r) => r.severity === selectedSeverity);

  const emailCritical = emailRules.filter((r) => r.severity === "CRITICAL").reduce((acc, r) => acc + r.issue_count, 0);
  const emailWarning = emailRules.filter((r) => r.severity === "WARNING").reduce((acc, r) => acc + r.issue_count, 0);
  const emailInfo = emailRules.filter((r) => r.severity === "INFO").reduce((acc, r) => acc + r.issue_count, 0);
      ...
      <View className="bg-dark-card border border-dark-border rounded-xl p-3 mb-4 flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <View className="flex-row items-center gap-4 flex-1">
          ...
          <View className="flex-1">
            <View className="flex-row items-center gap-2 mb-1">
              <Mail size={18} color={THEME_COLORS.primaryIcon} />
              <Text className="text-base font-bold text-white">Email Health & Formatting Audit</Text>
            </View>
            <Text className="text-xs text-slate-400 leading-relaxed">
              SSoT email qualification detecting RFC syntax errors, duplicate personal/work emails, public domains in corporate fields, and unassigned email profiles.
            </Text>
          </View>
        </View>
        ...
        <View className="flex-row gap-2.5 self-start md:self-auto">
          <View className="bg-red-950/40 border border-red-800/40 px-3.5 py-2 rounded-xl items-center min-w-[70px]">
            <Text className="text-lg font-black text-red-400">{emailCritical}</Text>
          ...
          <View className="bg-amber-950/40 border border-amber-800/40 px-3.5 py-2 rounded-xl items-center min-w-[70px]">
            <Text className="text-lg font-black text-amber-400">{emailWarning}</Text>
          ...
          <View className="bg-indigo-950/40 border border-indigo-800/40 px-3.5 py-2 rounded-xl items-center min-w-[70px]">
            <Text className="text-lg font-black text-indigo-400">{emailInfo}</Text>
          ...
          return (
            ...
            >
              <Text className={`text-xs font-bold ${isActive ? "text-white" : "text-slate-400"}`}>
                {sev === "ALL" ? `All Rules (${emailRules.length})` : sev}
              </Text>
            </TouchableOpacity>
          );
        })}
