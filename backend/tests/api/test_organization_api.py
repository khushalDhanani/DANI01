...
def test_get_org_overview_endpoint():
    mock_scale = [
        {
            "total_companies": 2,
            "active_companies": 2,
    ...
    with patch(
        ...
        res = client.get("/api/v1/modules/ORGANIZATION/overview")
        assert res.status_code == 200
        data = res.json()
        assert data["scale_counts"]["total_companies"] == 2
        assert data["active_employee_total"] == 1316


def test_get_org_hierarchy_endpoint():
    mock_rows = [
        {
            "CompID": 1,
    ...
    with patch("app.modules.organization.service.execute_readonly_query", return_value=mock_rows):
        res = client.get("/api/v1/modules/ORGANIZATION/hierarchy")
        assert res.status_code == 200
        data = res.json()
        assert len(data["companies"]) == 1
        assert data["companies"][0]["name"] == "Aether Industries Limited"


def test_get_org_units_endpoint():
    mock_count = [{"total": 1}]
    mock_items = [
        {
            "unit_id": 1,
    ...
    with patch(
        ...
        res = client.get("/api/v1/modules/ORGANIZATION/units?unit_type=COMPANY")
        assert res.status_code == 200
        data = res.json()
        assert data["total"] == 1
...
def test_get_org_quality_endpoint():
    mock_dq = [
    ...
    with patch("app.modules.organization.service.execute_readonly_query", return_value=mock_dq):
        res = client.get("/api/v1/modules/ORGANIZATION/quality")
        assert res.status_code == 200
        data = res.json()
...
def test_get_org_reporting_endpoint():
    mock_rows = [
        {
            "EmpID": 170,
            "EmpCode": "1001",
    ...
    with patch("app.modules.organization.service.execute_readonly_query", return_value=mock_rows):
        res = client.get("/api/v1/modules/ORGANIZATION/reporting")
        assert res.status_code == 200
        data = res.json()
