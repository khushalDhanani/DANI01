...
import { PayrollQualityTab } from "./PayrollQualityTab";
import { PayrollRegisterTab } from "./PayrollRegisterTab";

type TabKey = "overview" | "register" | "quality";
...
export function PayrollModuleScreen() {
  ...
  const tabs: { key: TabKey; label: string; icon: React.ReactNode; badge?: string }[] = [
    {
      key: "overview",
      label: "Overview",
      icon: <TrendingUp size={14} color={activeTab === "overview" ? "#ffffff" : "#94a3b8"} />,
    },
    {
      key: "register",
      label: "Monthly Salary Register",
      icon: <FileCheck size={14} color={activeTab === "register" ? "#ffffff" : "#94a3b8"} />,
    },
    {
      key: "quality",
  ...
  return (
    <ScrollView className="flex-1 bg-dark-bg p-3 md:p-4" showsVerticalScrollIndicator={false}>
      ...
      <View className="flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
        <View className="flex-1 flex-row items-center gap-3">
          <View className="bg-purple-950/80 border border-purple-800/60 px-2 py-0.5 rounded">
            <Text className="text-[10px] font-mono font-bold text-purple-400">PAYROLL</Text>
          </View>
          <View>
            ...
            <Text className="text-[11px] text-slate-400" numberOfLines={1}>
              Monthly registers, itemized earnings & deductions, payslips, and 6 audit rules.
            </Text>
          </View>
        </View>
      ...
      {/* ── Tab Content Views ───────────────────────────────── */}
      {activeTab === "overview" && <PayrollOverviewTab compId={selectedCompId} />}
      {activeTab === "register" && <PayrollRegisterTab compId={selectedCompId} />}
      {activeTab === "quality" && <PayrollQualityTab />}
    </ScrollView>
  );
}
